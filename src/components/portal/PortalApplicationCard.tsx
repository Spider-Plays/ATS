...
export function PortalApplicationCard({ app }: { app: PortalApplication }) {
  ...
  return (
    <article className="portal-dash-app overflow-hidden">
      ...
      {app.isCurrent && !isClosed && (
        <div className="portal-dash-app__body !pt-0">
          <p className="portal-dash-app__section-label">Your progress</p>
          <PortalPipelineTracker status={pipelineStatus} compact />
        </div>
      )}

      <div className="px-5 py-4 flex flex-wrap gap-3 border-t border-slate-100 bg-slate-50/80">
        <Link
          to={`/candidate/jobs/applied/${app.requirementId}`}
          className="btn-filled !h-9 !px-4 !text-sm inline-flex items-center gap-1"
        >
          Current updates
        ...
        {app.listedOnPortal && !isClosed && (
          <Link
            to={`/candidate/jobs/${app.requirementId}`}
            className="text-sm font-bold text-muted-foreground hover:text-primary py-2.5"
          >
            Job posting →
          </Link>
        )}
